...
namespace kmuvcl {
  namespace math {
    ...
    vec<N, T> operator+ (const vec<N, T>& u, const vec<N, T>& v) //행렬덧셈
    {
      vec<N, T>  w;
    ...
    /// w_n = u_n - v_n
    template <unsigned int N, typename T>
    vec<N, T> operator- (const vec<N, T>& u, const vec<N, T>& v) //행렬뺄셈
    {
      vec<N, T>  w;
    ...
    /// y_n = s * x_n
    template <unsigned int N, typename T>
    vec<N, T> operator* (const T s, const vec<N, T>& x) //행렬의 실수베
    {
      vec<N, T>  y;
